...
/**
 * Create regular expression string pattern.
 */
...
const char *POLYGON_LINE_PATTERN		= "f[ \b][\\ ]*([0-9]+[\\/][0-9]+[\\/][0-9]+[\\ ]*){5,}";
...
void compile_regular_expressions(Regular* regular)
{
    ...
    else if (regcomp(&regular->polygon_regex, POLYGON_LINE_PATTERN, REG_EXTENDED) != 0)
    {
        fprintf(stderr, "Failed to compile regex '%s'\n", POLYGON_LINE_PATTERN);
        return ;
    }
}
...
bool is_polygon_line(const Regular* const regular, const char* const line)
{
    ...
    retval = regexec(&regular->polygon_regex, line, 2, rm, 0);
    return retval == 0;
}
...
void count_elements(FILE* file,Model* model,Regular* regular)
{
    ...
    compile_regular_expressions(regular);
	...
	while (fgets(line, BUFFER_SIZE, file) != NULL)
    {
        ...
        {
            model->n_faces++;
            	if	((retval = regexec(&regular->polygon_regex, line, 2, rm, 0)) == 0)
       		{	
            	model->n_polygons++;
		}
